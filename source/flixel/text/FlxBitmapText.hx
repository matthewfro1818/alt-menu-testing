...
class FlxBitmapText extends FlxSprite
{
	...
	/**
	 * Wraps provided line by characters (as in standart openfl text fields).
	 *
	 * @param	words		The array of words in the line to process.
	 * @param	newLines	Array to fill with result lines.
	 */
		...
		for (i in 0...lineLength)
		{
			...
			else
			{
				...
				if (charFrame != null)
				{
					...
					{
						_openflPoint.setTo(curX - font.getCharAdvance(charCode) - letterSpacing, curY);
					}
					else
					{
						_openflPoint.setTo(curX, curY);
						curX += font.getCharAdvance(charCode) + letterSpacing;
					}
					charFrame.paint(textBitmap, _openflPoint, true);
	...
	function blitText(posX:Int, posY:Int, isFront:Bool = true, ?bitmap:BitmapData):Void
	{
		...
		if (isFront && !useTextColor)
		{
			_openflRect.setTo(0, 0, textBitmap.width, textBitmap.height);
			bitmap.copyPixels(textBitmap, _openflRect, _openflPointZero, null, null, true);
		}
		else
		{
	 ...
	 * @param	Color outline color in openfl 0xAARRGGBB format
