...
class FlxText extends FlxSprite
{
	...
	function regenGraphic():Void
	{
		...
		// prevent text height from shrinking on openfl if text == ""
		...
		if (oldWidth != newWidth || oldHeight != newHeight)
		{
			...
			_openflRect.x = 0;
			_openflRect.y = 0;
			_openflRect.width = newWidth;
			_openflRect.height = newHeight;
		}
		else // Else just clear the old buffer before redrawing the text
		{
			graphic.bitmap.fillRect(_openflRect, FlxColor.TRANSPARENT);
			if (_hasBorderAlpha)
			{
				if (_borderPixels == null)
					_borderPixels = new BitmapData(frameWidth, frameHeight, true);
				else
					_borderPixels.fillRect(_openflRect, FlxColor.TRANSPARENT);
			}
		}
	...
	/**
	 * Internal function to draw textField to a BitmapData, if openfl it calculates every line x to avoid blurry lines.
	 */
	function drawTextFieldTo(graphic:BitmapData):Void
	{
		...
		if (alignment == FlxTextAlign.CENTER && isTextBlurry())
		{
			...
			for (i in 0...textField.numLines)
			{
				...
				// Workaround for blurry lines caused by non-integer x positions on openfl
...
abstract FlxTextAlign(String) from String
{
	...
	/**
	 * Warning: on openfl, this can have a negative impact on performance
