...
class FlxTilemapBuffer implements IFlxDestroyable
{
	...
	public function resize(TileWidth:Int, TileHeight:Int, WidthInTiles:Int, HeightInTiles:Int, ?Camera:FlxCamera, ScaleX:Float = 1.0, ScaleY:Float = 1.0):Void
	{
		...
		if (FlxG.renderBlit)
		{
			...
			if (pixels == null)
			{
				...
				_openflRect = new Rectangle(0, 0, newWidth, newHeight);
			...
			else if (pixels.width != newWidth || pixels.height != newHeight)
			{
				...
				pixels = new BitmapData(newWidth, newHeight, true, 0);
				_openflRect.setTo(0, 0, newWidth, newHeight);
				dirty = true;
			}
		}
	}
	...
	public function fill(Color:FlxColor = FlxColor.TRANSPARENT):Void
	{
		if (FlxG.renderBlit)
		{
			pixels.fillRect(_openflRect, Color);
		}
	}
	 ...
	 * @param	openflPoint	Where to draw the buffer at in camera coordinates.
	 */
	public function draw(Camera:FlxCamera, openflPoint:Point, ScaleX:Float = 1.0, ScaleY:Float = 1.0):Void
	{
		if (isPixelPerfectRender(Camera))
		{
			openflPoint.x = Math.floor(openflPoint.x);
			openflPoint.y = Math.floor(openflPoint.y);
		}

		if (isPixelPerfectRender(Camera) && (ScaleX == 1.0 && ScaleY == 1.0) && blend == null)
		{
			Camera.copyPixels(pixels, _openflRect, openflPoint, null, null, true);
		}
		else
		{
			_matrix.identity();
			_matrix.scale(ScaleX, ScaleY);
			_matrix.translate(openflPoint.x, openflPoint.y);
	...
	{
		pixels.colorTransform(_openflRect, Transform);
	}
