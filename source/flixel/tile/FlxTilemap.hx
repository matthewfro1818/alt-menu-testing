...
class FlxTilemap extends FlxBaseTilemap<FlxTile>
{
	 ...
	 * Rendering helper, minimize new object instantiation on repetitive methods.
	 */
	var _openflRect:Rectangle = new Rectangle();
	...
	function set_frames(value:FlxFramesCollection):FlxFramesCollection
	{
		...
		if (value != null)
		{
			...
			_openflRect.setTo(0, 0, _tileWidth, _tileHeight);
	...
	override public function draw():Void
	{
		...
		for (i in 0...l)
		{
			...
			if (FlxG.renderBlit)
			{
				...
				getScreenPosition(_point, camera).subtractPoint(offset).add(buffer.x, buffer.y).copyToopenfl(_openflPoint);
				buffer.draw(camera, _openflPoint, scale.x, scale.y);
			}
			else
			{
				drawTilemap(buffer, camera);
	...
	function drawTilemap(Buffer:FlxTilemapBuffer, Camera:FlxCamera):Void
	{
		...
		{
			getScreenPosition(_point, Camera).subtractPoint(offset).copyToopenfl(_helperPoint);
		...
		for (row in 0...screenRows)
		{
			...
			for (column in 0...screenColumns)
			{
				...
				if (tile != null && tile.visible && tile.frame.type != FlxFrameType.EMPTY)
				{
					...
					if (FlxG.renderBlit)
					{
						frame.paint(Buffer.pixels, _openflPoint, true);
						...
						if (FlxG.debugger.drawDebug && !ignoreDrawDebug)
						{
							...
							offset.addToopenfl(_openflPoint);
							Buffer.pixels.copyPixels(debugTile, _debugRect, _openflPoint, null, null, true);
							offset.subtractFromopenfl(_openflPoint);
						}
						#end
					}
					else
				...
				if (FlxG.renderBlit)
					_openflPoint.x += _tileWidth;

				columnIndex++;
			}

			if (FlxG.renderBlit)
				_openflPoint.y += _tileHeight;
			rowIndex += widthInTiles;
		}
	...
	function drawDebugTile(debugTile:BitmapData, color:FlxColor):Void
	{
		if (color != FlxColor.TRANSPARENT)
		{
			...
			debugTile.draw(FlxSpriteUtil.openflGfxSprite);
		}
	}
