...
class FlxDefines
{
	...
	{
		if (!defined(FLX_NO_MOUSE) && !defined(FLX_NO_MOUSE_ADVANCED) && (!defined("openfl") || defined("openfl11_2")))
			define(FLX_MOUSE_ADVANCED);

		if (!defined(FLX_NO_MOUSE) && !defined(FLX_NO_NATIVE_CURSOR) && defined("openfl10_2"))
			define(FLX_NATIVE_CURSOR);

		if (!defined(FLX_NO_SOUND_SYSTEM) && !defined(FLX_NO_SOUND_TRAY))
			define(FLX_SOUND_TRAY);

		if ((!defined("openfl_legacy") && !defined("openfl")) || defined("openfl11_8"))
	...
	{
		if (!defined("openfl11"))
			abort("The minimum required openfl Player version for HaxeFlixel is 11." + " Please specify a newer version in your Project.xml file.",
				(macro null).pos);
	...
	{
		var errorMessage = '$feature only supported in openfl Player version $version or higher. '
			+ 'Define ${define.getName()} to disable this feature or add <set name="SWF_VERSION" value="$version" /> to your Project.xml.';

		if (!defined("openfl" + version.replace(".", "_")) && !defined(define))
