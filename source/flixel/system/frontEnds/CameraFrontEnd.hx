...
class CameraFrontEnd
{
	...
	{
		FlxG.game.addChildAt(NewCamera.openflSprite, FlxG.game.getChildIndex(FlxG.game._inputContainer));
	...
	public function remove(Camera:FlxCamera, Destroy:Bool = true):Void
	{
		...
		{
			FlxG.game.removeChild(Camera.openflSprite);
			list.splice(index, 1);
	...
	public function openfl(Color:FlxColor = FlxColor.WHITE, Duration:Float = 1, ?OnComplete:Void->Void, Force:Bool = false):Void
	{
		for (camera in list)
		{
			camera.openfl(Color, Duration, OnComplete, Force);
		}
	}
