 ...
 * `openflSprite:Sprite` (which is a container for everything else in the camera, it's added to FlxG.game sprite)
 *     |-> `_scrollRect:Sprite` (which is used for cropping camera's graphic, mostly in tile render mode)
 *         |-> `_openflBitmap:Bitmap`  (its bitmapData property is buffer BitmapData, this var is used in blit render mode.
...
class FlxCamera extends FlxBasic
{
	 ...
	 * On openfl, transparent backgrounds can be used in conjunction with `useBgAlphaBlending`.
	 */
	 ...
	 *
	 * Its position is modified by `updateopenflSpritePosition()` which is called every frame.
	 */
	public var openflSprite:Sprite = new Sprite();
	 ...
	 * Do not modify it unless you know what are you doing.
	 */
	var _openflPoint:Point = new Point();

	/**
	 * Internal, used for positioning camera's `openflSprite` on screen.
	 ...
	 * Do not modify it unless you know what are you doing.
	 */
	var _openflOffset:FlxPoint = FlxPoint.get();

	/**
	 * Internal, represents the color of `openfl()` special effect.
	 */
	var _fxopenflColor:FlxColor = FlxColor.TRANSPARENT;

	/**
	 * Internal, stores `openfl()` special effect duration.
	 */
	var _fxopenflDuration:Float = 0;

	/**
	 * Internal, camera's `openfl()` complete callback.
	 */
	var _fxopenflComplete:Void->Void = null;

	/**
	 * Internal, used to control the `openfl()` special effect.
	 */
	var _fxopenflAlpha:Float = 0;
	 ...
	 * This Bitmap used for rendering camera's buffer (`_openflBitmap.bitmapData = buffer;`)
	...
	/**
	 * Sprite used for actual rendering in tile render mode (instead of `_openflBitmap` for blitting).
	...
	#if FLX_DEBUG
	/**
	 * Sprite for visual effects (openfl and fade) and drawDebug information
			...
			?position:FlxPoint, ?blend:BlendMode, repeat:Bool = false, smoothing:Bool = false):Void
	{
		if (FlxG.renderBlit)
		{
			...
			else
			{
				...
				{
					var gfx:Graphics = FlxSpriteUtil.openflGfx;
					gfx.clear();
					gfx.lineStyle(1, FlxColor.BLUE, 0.5);
					gfx.drawTriangles(drawVertices, indices);
					camera.buffer.draw(FlxSpriteUtil.openflGfxSprite, _helperMatrix);
				}
				#end
				// End of TODO...
			}
	...
	public function new(X:Int = 0, Y:Int = 0, Width:Int = 0, Height:Int = 0, Zoom:Float = 0)
	{
		...
		_openflRect = new Rectangle(0, 0, width, height);

		openflSprite.addChild(_scrollRect);
		...
		if (FlxG.renderBlit)
		{
			...
			screen.origin.set();
			_openflBitmap = new Bitmap(buffer);
			_scrollRect.addChild(_openflBitmap);
		...
		zoom = Zoom; // sets the scale of openfl sprite, which in turn loads openflOffset values

		updateScrollRect();
		updateopenflOffset();
		updateopenflSpritePosition();
	...
	{
		FlxDestroyUtil.removeChild(openflSprite, _scrollRect);

		if (FlxG.renderBlit)
		{
			FlxDestroyUtil.removeChild(_scrollRect, _openflBitmap);
	...
	override public function update(elapsed:Float):Void
	{
		...
		updateScroll();
		updateopenfl(elapsed);
		updateFade(elapsed);

		openflSprite.filters = filtersEnabled ? _filters : null;
	...
	function updateopenfl(elapsed:Float):Void
	{
		// Update the "openfl" special effect
		if (_fxopenflAlpha > 0.0)
		{
			_fxopenflAlpha -= elapsed / _fxopenflDuration;
			if ((_fxopenflAlpha <= 0) && (_fxopenflComplete != null))
			{
				_fxopenflComplete();
			}
		}
	...
	function updateShake(elapsed:Float):Void
	{
		if (_fxShakeDuration > 0)
		{
			...
			else
			{
				...
				{
					openflSprite.x += FlxG.random.float(-_fxShakeIntensity * width, _fxShakeIntensity * width) * zoom * FlxG.scaleMode.scale.x;
				}
				if (_fxShakeAxes != FlxAxes.X)
				{
					openflSprite.y += FlxG.random.float(-_fxShakeIntensity * height, _fxShakeIntensity * height) * zoom * FlxG.scaleMode.scale.y;
				}
			}
		}
	}
	...
	/**
	 * Recalculates `openflSprite` position.
	 * Called every frame by camera's `update()` method and every time you change camera's position.
	 */
	function updateopenflSpritePosition():Void
	{
		if (openflSprite != null)
		{
			openflSprite.x = x * FlxG.scaleMode.scale.x + _openflOffset.x;
			openflSprite.y = y * FlxG.scaleMode.scale.y + _openflOffset.y;
		}
	}

	/**
	 * Recalculates `_openflOffset` point, which is used for positioning openflSprite in the game.
	 * It's called every time you resize the camera or the game.
	 */
	function updateopenflOffset():Void
	{
		_openflOffset.x = width * 0.5 * FlxG.scaleMode.scale.x * initialZoom;
		_openflOffset.y = height * 0.5 * FlxG.scaleMode.scale.y * initialZoom;
	}

	/**
	 * Updates `_scrollRect` sprite to crop graphics of the camera:
	 * 1) `scrollRect` property of this sprite
	 * 2) position of this sprite inside `openflSprite`
	 *
	...
	/**
	 * Modifies position of `_openflBitmap` in blit render mode and `canvas` and `debugSprite`
	 ...
	 * @param   Duration     How long it takes for the openfl to fade.
	 * @param   OnComplete   A function you want to run when the openfl finishes.
	 * @param   Force        Force the effect to reset.
	 */
	public function openfl(Color:FlxColor = FlxColor.WHITE, Duration:Float = 1, ?OnComplete:Void->Void, Force:Bool = false):Void
	{
		if (!Force && (_fxopenflAlpha > 0.0))
			return;

		_fxopenflColor = Color;
		if (Duration <= 0)
			Duration = 0.000001;
		_fxopenflDuration = Duration;
		_fxopenflComplete = OnComplete;
		_fxopenflAlpha = 1.0;
	}
	 ...
	 * @param   Force        Force the effect to reset (default = `true`, unlike `openfl()` and `fade()`!).
	...
	public function fill(Color:FlxColor, BlendAlpha:Bool = true, FxAlpha:Float = 1.0, ?graphics:Graphics):Void
	{
		if (FlxG.renderBlit)
		{
			if (BlendAlpha)
			{
				_fill.fillRect(_openflRect, Color);
				buffer.copyPixels(_fill, _openflRect, _openflPoint, null, null, BlendAlpha);
			}
			else
			{
				buffer.fillRect(_openflRect, Color);
			}
		}
		else
		{
	...
	function drawFX():Void
	{
		...
		// Draw the "openfl" special effect onto the buffer
		if (_fxopenflAlpha > 0.0)
		{
			alphaComponent = _fxopenflColor.alpha;

			if (FlxG.renderBlit)
			{
				fill((Std.int(((alphaComponent <= 0) ? 0xff : alphaComponent) * _fxopenflAlpha) << 24) + (_fxopenflColor & 0x00ffffff));
			}
			else
			{
				fill((_fxopenflColor & 0x00ffffff), true, ((alphaComponent <= 0) ? 0xff : alphaComponent) * _fxopenflAlpha / 255, canvas.graphics);
			}
		}
	...
	function checkResize():Void
	{
		if (FlxG.renderBlit)
		{
			if (width != buffer.width || height != buffer.height)
			{
				...
				screen.pixels = buffer;
				screen.origin.set();
				_openflBitmap.bitmapData = buffer;
				_openflRect.width = width;
				_openflRect.height = height;
				_fill = FlxDestroyUtil.dispose(_fill);
				_fill = new BitmapData(width, height, true, FlxColor.TRANSPARENT);
	...
	public function setScale(X:Float, Y:Float):Void
	{
		...
		if (FlxG.renderBlit)
		{
			...
			if (_useBlitMatrix)
			{
				_openflBitmap.scaleX = initialZoom * FlxG.scaleMode.scale.x;
				_openflBitmap.scaleY = initialZoom * FlxG.scaleMode.scale.y;
			}
			else
			{
				_openflBitmap.scaleX = totalScaleX;
				_openflBitmap.scaleY = totalScaleY;
			}
		}
	...
	function set_color(Color:FlxColor):FlxColor
	{
		...
		if (FlxG.renderBlit)
		{
			...
			colorTransform = _openflBitmap.transform.colorTransform;
		}
		else
		{
		...
		if (FlxG.renderBlit)
		{
			_openflBitmap.transform.colorTransform = colorTransform;
		}
		else
		{
	...
	function set_antialiasing(Antialiasing:Bool):Bool
	{
		...
		if (FlxG.renderBlit)
		{
			_openflBitmap.smoothing = Antialiasing;
		}
		return Antialiasing;
	}
