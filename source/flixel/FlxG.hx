...
class FlxG
{
	...
	/**
	 * The openfl stage object (required for event listeners).
	...
	/**
	 * Contains things related to cameras, a list of all cameras and several effects like `openfl()` or `fade()`.
	 */
	...
	static function initRenderMethod():Void
	{
		...
		else
		{
			renderMethod = switch (stage.window.renderer.type)
			{
				...
				case CANVAS, openfl, CAIRO: BLITTING;
				default: BLITTING;
			}
		}
		#end
	...
	static function set_updateFramerate(Framerate:Int):Int
	{
		if (Framerate < drawFramerate)
			log.warn("FlxG.framerate: the game's framerate shouldn't be smaller than the openfl framerate," + " since it can stop your game from updating.");
