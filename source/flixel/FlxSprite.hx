...
class FlxSprite extends FlxObject
{
	...
	override function initVars():Void
	{
		...
		_openflPoint = new Point();
		_openflRect = new Rectangle();
		_openflRect2 = new Rectangle();
		_openflPointZero = new Point();
		offset = FlxPoint.get();
		origin = FlxPoint.get();
	 ...
	 * This method can useful for non-openfl targets.
	 *
			...
			?Key:String):FlxSprite
	{
		...
		if (Frame >= 0)
		{
			...
			_openflRect.setTo(Frame * brushSize, 0, brushSize, brushSize);
			brush.copyPixels(full, _openflRect, _openflPointZero);
		}
	...
	public inline function resetSize():Void
	{
		_openflRect.x = 0;
		_openflRect.y = 0;
		_openflRect.width = frameWidth;
		_openflRect.height = frameHeight;
	}
	...
	function resetHelpers():Void
	{
		...
		if (graphic != null)
		{
			_openflRect2.width = graphic.width;
			_openflRect2.height = graphic.height;
		}
	...
	function drawSimple(camera:FlxCamera):Void
	{
		...
		_point.copyToopenfl(_openflPoint);
		camera.copyPixels(_frame, framePixels, _openflRect, _openflPoint, colorTransform, blend, antialiasing);
	}
	...
	public function stamp(Brush:FlxSprite, X:Int = 0, Y:Int = 0):Void
	{
		...
		{
			_openflPoint.x = X + frame.frame.x;
			_openflPoint.y = Y + frame.frame.y;
			_openflRect2.width = bitmapData.width;
			_openflRect2.height = bitmapData.height;
			graphic.bitmap.copyPixels(bitmapData, _openflRect2, _openflPoint, null, null, true);
			_openflRect2.width = graphic.bitmap.width;
			_openflRect2.height = graphic.bitmap.height;
		}
		else // complex render
		{
			_matrix.identity();
	...
	public function pixelsOverlapPoint(point:FlxPoint, Mask:Int = 0xFF, ?Camera:FlxCamera):Bool
	{
		...
		_openflPoint.x = (point.x - Camera.scroll.x) - _point.x;
		_openflPoint.y = (point.y - Camera.scroll.y) - _point.y;

		point.putWeak();

		// 1. Check to see if the point is outside of framePixels rectangle
		if (_openflPoint.x < 0 || _openflPoint.x > frameWidth || _openflPoint.y < 0 || _openflPoint.y > frameHeight)
		{
			return false;
		}
		else // 2. Check pixel at (_openflPoint.x, _openflPoint.y)
		{
			var frameData:BitmapData = updateFramePixels();
			var pixelColor:FlxColor = frameData.getPixel32(Std.int(_openflPoint.x), Std.int(_openflPoint.y));
	 ...
	 *
	 * @param   RunOnCpp   Whether the frame should also be recalculated if we're on a non-openfl target
	 */
	@:noCompletion
	...
	public function updateFramePixels():BitmapData
	{
		...
		{
			framePixels = _frame.paint(framePixels, _openflPointZero, false, true);
		}
		else
		{
			framePixels = _frame.paintRotatedAndFlipped(framePixels, _openflPointZero, FlxFrameAngle.ANGLE_0, doFlipX, doFlipY, false, true);
		}

		if (useColorTransform)
		{
			framePixels.colorTransform(_openflRect, colorTransform);
		}
