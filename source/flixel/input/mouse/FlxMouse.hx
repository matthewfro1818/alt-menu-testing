...
class FlxMouse extends FlxPointer implements IFlxInputManager
{
	 ...
	 * sits at the right "height". Not used on openfl with the native cursor API.
	 */
	...
	/**
	 * Helper variables for openfl native cursors
	 */
	#if FLX_NATIVE_CURSOR
	...
	/**
	 * Load a new mouse cursor graphic - if you're using native cursors on openfl,
	...
	public function setNativeCursor(name:String):Void
	{
		...
		// openfl requires the use of AUTO before a custom cursor to work
		Mouse.cursor = MouseCursor.AUTO;
		Mouse.cursor = _currentNativeCursor;
	}

	/**
	 * Shortcut to register a native cursor in openfl
	 *
	 ...
	 */
	function onMouseWheel(openflEvent:MouseEvent):Void
	{
		if (enabled)
		{
			_wheelUsed = true;
			wheel = openflEvent.delta;
		}
	}
