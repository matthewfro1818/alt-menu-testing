...
class FlxTouchManager implements IFlxInputManager
{
	 ...
	 */
	function handleTouchBegin(openflEvent:TouchEvent):Void
	{
		var touch:FlxTouch = _touchesCache.get(openflEvent.touchPointID);
		if (touch != null)
		{
			touch.setXY(Std.int(openflEvent.stageX), Std.int(openflEvent.stageY));
		}
		else
		{
			touch = recycle(Std.int(openflEvent.stageX), Std.int(openflEvent.stageY), openflEvent.touchPointID);
		}
		touch.input.press();
	}
	 ...
	 * Event handler so FlxGame can update touches.
	 */
	function handleTouchEnd(openflEvent:TouchEvent):Void
	{
		var touch:FlxTouch = _touchesCache.get(openflEvent.touchPointID);
	 ...
	 * Event handler so FlxGame can update touches.
	 */
	function handleTouchMove(openflEvent:TouchEvent):Void
	{
		var touch:FlxTouch = _touchesCache.get(openflEvent.touchPointID);
