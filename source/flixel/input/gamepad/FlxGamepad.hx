...
import flixel.input.gamepad.mappings.MayopenflWiiRemoteMapping;
...
class FlxGamepad implements IFlxDestroyable
{
	 ...
	 * (should be used in openfl to correct the inverted y axis)
	 */
	...
	function createMappingForModel(model:FlxGamepadModel):FlxGamepadMapping
	{
		return switch (model)
		{
			...
			case MAYopenfl_WII_REMOTE: new MayopenflWiiRemoteMapping(attachment);
