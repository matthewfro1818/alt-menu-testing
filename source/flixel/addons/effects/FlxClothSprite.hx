...
class FlxClothSprite extends FlxSprite
{
	...
	/**
	 * The actual openfl BitmapData object representing the current display state of the modified framePixels.
	 */
	...
	override function drawSimple(camera:FlxCamera):Void
	{
		...
		_point.addPoint(_drawOffset).copyToopenfl(_openflPoint);
		camera.copyPixels(_frame, meshPixels, meshPixels.rect, _openflPoint, colorTransform, blend, antialiasing);
	}
	...
	function drawImage():Void
	{
		if (meshPixels != null)
		{
			FlxSpriteUtil.openflGfx.clear();
			FlxSpriteUtil.openflGfx.beginBitmapFill(framePixels, null, false, true);
			FlxSpriteUtil.openflGfx.drawTriangles(_vertices, _indices, _uvtData);
			FlxSpriteUtil.openflGfx.endFill();

			meshPixels.draw(FlxSpriteUtil.openflGfxSprite);
		}
	}
}
