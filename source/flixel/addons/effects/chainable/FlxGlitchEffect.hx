...
class FlxGlitchEffect implements IFlxEffect
{
	...
	/**
	 * The actual openfl BitmapData object representing the current effect state.
	 */
	...
	public function apply(bitmapData:BitmapData):BitmapData
	{
		if (_time == 0)
		{
			...
			if (direction == HORIZONTAL)
			{
				...
				{
					_openflRect.setTo(0, p, bitmapData.width, size);
					if (_openflRect.bottom > bitmapData.height)
						_openflRect.bottom = bitmapData.height;
					_openflPoint.setTo(FlxG.random.int(-strength, strength) + strength, p);
					p += Std.int(_openflRect.height);
					_pixels.copyPixels(bitmapData, _openflRect, _openflPoint);
				}
			}
			else
			{
				...
				{
					_openflRect.setTo(p, 0, size, bitmapData.height);
					if (_openflRect.right > bitmapData.width)
						_openflRect.right = bitmapData.width;
					_openflPoint.setTo(p, FlxG.random.int(-strength, strength) + strength);
					p += Std.int(_openflRect.width);
					_pixels.copyPixels(bitmapData, _openflRect, _openflPoint);
				}
			}
		}
