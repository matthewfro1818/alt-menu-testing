 ...
 * Note: Do NOT store you private key as an unobfuscated string! One method is to save it as a text file called "myKey.privatekey" and add "*.privatekey" to your ignore list for version control (.gitignore for git, global-ignores in your config file for svn, .hgignore for Mercurial). Then:
 	 ...
 	 * If your game is embedded as openfl on GameJolt's site, or run via Quick Play, you do not need to get the user name and token; this will be done automatically.
 	 ...
 	 * Then, verify this data via the following method:
 		 ...
 		 * FlxGameJolt.init(gameid, keystring); // Use this if your game is embedded as openfl on GameJolt's site, or run via Quick Play. If
 */
class FlxGameJolt
{
	/**
	...
	/**
	 * An alternative to running authUser() and hoping for the best; this will tell you if your game was run as an embedded openfl on GameJolt that has user name and token data already. Does NOT authenticate the user data!
	 *
	 * @return	True if it's an embedded SWF with user name and token available, false otherwise.
	 */
	public static var isEmbeddedopenfl(get, never):Bool;
	...
	/**
	 * Internal storage for this user's username. Can be retrieved automatically if openfl or QuickPlay.
	 */
	static var _userName:String;

	/**
	 * Internal storage for this user's token. Can be retrieved automatically if openfl or QuickPlay.
	 */
	 ...
	 * @param 	UserName	The username to authenticate, if AutoAuth is true. If you set AutoAuth to true but don't put a value here, FlxGameJolt will attempt to get the user data automatically, which will only work for openfl embedded on GameJolt, or desktop games run via Quick Play.
	 * @param 	UserToken	The user token to authenticate, if AutoAuth is true. If you set AutoAuth to true but don't put a value here, FlxGameJolt will attempt to get the user data automatically, which will only work for openfl embedded on GameJolt, or desktop games run via Quick Play.
	...
	public static function init(GameID:Int, PrivateKey:String, AutoAuth:Bool = false, ?UserName:String, ?UserToken:String, ?Callback:Dynamic):Void
	{
		...
		// OR it must be embedded openfl or quickplay.
		...
		if (AutoAuth)
		{
			...
			else if ((UserName == null || UserToken == null) && (isEmbeddedopenfl || isQuickPlay))
			{
	 ...
	 * @param	UserName	A user name. Leave null to automatically pull user data (only works for embedded openfl on GameJolt or Quick Play). Usernames can only have letters, numbers, hyphens (-) and underscores (_), and must be 3-30 characters long.
	 * @param	UserToken	A user token. Players enter this instead of a password to enable highscores, trophies, etc. Leave null to automatically pull user data (only works for embedded openfl on GameJolt or Quick Play). User tokens can only have letters and numbers, and must be 4-30 characters long.
	...
	static function get_isEmbeddedopenfl():Bool
	{
		#if openfl
		var parameters = Lib.current.loaderInfo.parameters;
