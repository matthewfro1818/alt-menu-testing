		...
		implements IHasParams implements ICursorPointable
{
		...
		if (assets.length == 1)
		{
			...
			if (multiFrame)
			{
				...
				if (slice9 != null && slice9[0] != [])
				{
					...
					for (i in 0...arr_bmpData.length)
					{
						arr_flx9[i] = new FlxUI9SliceSprite(0, 0, arr_bmpData[i], _Rect2, slice9[0], tile, false, assets[0] + ":" + i, resize_ratio);
		...
		else
		{
			...
			if (assets[0] != "")
			{
				if (slice9 != null && slice9.length > 0 && slice9[0] != null && slice9[0].length > 0)
				{
					...
					if (slice9[0] != null)
					{
						arr_flx9[0] = new FlxUI9SliceSprite(0, 0, assets[0], _Rect2, slice9[0], tile, false, "", resize_ratio);
					}
					else
					{
					...
					for (i in 1...assets.length)
					{
						if (assets[i] != "")
						{
							if (slice9[i] != null)
							{
								arr_flx9[i] = new FlxUI9SliceSprite(0, 0, assets[i], _Rect2, slice9[i], tile, false, "", resize_ratio);
							}
							else
							{
			...
			?key:String = null):BitmapData
	{
		...
		_Rect.x = 0;
		_Rect.y = button_state * h;
		_Rect.width = w;
		_Rect.height = h;
		if (_Rect.y >= all_frames.height)
		{
			...
			if (framesHigh == 4)
			{
				...
				{
					// toggle-hilight
					_Rect.y = (3) * h; // show "down_over"
				}
				else if (button_state == FlxButtonState.PRESSED + 3)
				{
					// toggle-pressed
					_Rect.y = (2) * h; // show "down"
				}
			}
		}

		// Check to see if we can return the cached image instead
		var frameKey = key + "{x:" + _Rect.x + "y:" + _Rect.y + "w:" + _Rect.width + "h:" + _Rect.height + "}";
		if (frameKey != null)
		{
		...
		pixels.copyPixels(all_frames, _Rect, _PointZero);
		if (key != null)
		{
	...
	public function combineToggleBitmaps(normal:BitmapData, toggle:BitmapData):BitmapData
	{
		...
		combined.copyPixels(normal, normal.rect, _PointZero);
		_Point.x = 0;
		_Point.y = normal.height;
		combined.copyPixels(toggle, toggle.rect, _Point);
	...
	public function assembleButtonFrames(upB:BitmapData, overB:BitmapData, downB:BitmapData):BitmapData
	{
		...
		pixels.copyPixels(upB, upB.rect, _PointZero);

		if (overB != null)
		{
			_Point.x = 0;
			_Point.y = upB.height;
			pixels.copyPixels(overB, overB.rect, _Point);
			if (downB != null)
			{
				_Point.y = upB.height * 2;
				pixels.copyPixels(downB, downB.rect, _Point);
			}
		}
