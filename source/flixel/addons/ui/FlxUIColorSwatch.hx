		...
		if (multiColored)
		{
			if (graphic.key != key)
			{
				if (FlxG.bitmap.checkCache(key) == false) // draw the swatch dynamically from supplied color values
				{
					...
					_Rect.x = 0;
					_Rect.y = 0;
					_Rect.width = pixels.width;
					_Rect.height = pixels.height;
					pixels.fillRect(_Rect, 0xFF000000); // start with black outline
					...
					for (i in 0...tempCols.length)
					{
						...
						pixels.fillRect(_Rect, col);
						_Rect.width -= (thickW * 2);
						_Rect.height -= (thickH * 2);
						_Rect.x += thickW;
						_Rect.y += thickH;
					}
