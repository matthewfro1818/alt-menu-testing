...
class FlxPieDial extends FlxSprite
{
	...
	function makePieDialGraphicSub(Radius:Int, Color:Int, Frames:Int, Shape:FlxPieDialShape, Clockwise:Bool, InnerRadius):BitmapData
	{
		...
		for (r in 0...rows)
		{
			for (c in 0...cols)
			{
				...
				_openflPoint.setTo(c * W, r * H);
				bmp2.copyPixels(fullBmp, fullBmp.rect, _openflPoint);
				...
				{
					nextFrame.pixels.copyPixels(fullFrame.pixels, fullFrame.pixels.rect, _openflPointZero);
					_openflPoint.setTo(c * W, r * H);
					drawSweep(sweep, v, nextFrame, polygon, W, H, back, fore);
					bmp.copyPixels(nextFrame.pixels, nextFrame.pixels.rect, _openflPoint);
				}
		...
		shapeChannel.copyChannel(bmp2, bmp2.rect, _openflPointZero, BitmapDataChannel.ALPHA, BitmapDataChannel.RED);
		shapeChannel.copyChannel(bmp2, bmp2.rect, _openflPointZero, BitmapDataChannel.ALPHA, BitmapDataChannel.GREEN);
		shapeChannel.copyChannel(bmp2, bmp2.rect, _openflPointZero, BitmapDataChannel.ALPHA, BitmapDataChannel.BLUE);

		shapeChannel.draw(bmp, null, null, BlendMode.MULTIPLY, null, true);
		bmp2.copyChannel(shapeChannel, shapeChannel.rect, _openflPointZero, BitmapDataChannel.RED, BitmapDataChannel.ALPHA);
	...
	function makeFullFrame(Radius:Int, Color:Int, Frames:Int, Shape:FlxPieDialShape, Clockwise:Bool, InnerRadius):FlxSprite
	{
		...
		if (Shape == SQUARE)
		{
			...
			if (InnerRadius > 0)
			{
				_openflRect.setTo(dR, dR, InnerRadius * 2, InnerRadius * 2);
				fullFrame.pixels.fillRect(_openflRect, FlxColor.TRANSPARENT);
			}
		}
		else if (Shape == CIRCLE)
		{
			if (InnerRadius > 0)
			{
				...
				fullFrame.pixels.fillRect(_openflRect, FlxColor.BLACK);
				...
				alpha.copyPixels(fullFrame.pixels, fullFrame.pixels.rect, _openflPointZero, null, null, true);

				fullFrame.pixels.fillRect(fullFrame.pixels.rect, Color);
				fullFrame.pixels.copyChannel(alpha, alpha.rect, _openflPointZero, BitmapDataChannel.RED, BitmapDataChannel.ALPHA);
	...
	function drawSweep(sweep:Float, v:FlxVector, nextFrame:FlxSprite, polygon:Array<FlxPoint>, W:Int, H:Int, back:FlxColor, fore:FlxColor)
	{
		...
		{
			_openflRect.setTo(halfW, 0, halfW, halfH);
			nextFrame.pixels.fillRect(_openflRect, fore);
		...
		{
			_openflRect.setTo(halfW, 0, halfW, H);
			nextFrame.pixels.fillRect(_openflRect, fore);
		...
		else if (sweep < 315)
		{
			_openflRect.setTo(halfW, 0, halfW, H);
			nextFrame.pixels.fillRect(_openflRect, fore);
			_openflRect.setTo(0, halfH, halfW, halfH);
			nextFrame.pixels.fillRect(_openflRect, fore);
