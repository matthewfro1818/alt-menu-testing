...
class FlxBackdrop extends FlxSprite
{
	...
	override public function draw():Void
	{
		...
		for (camera in cameras)
		{
			...
			if (FlxG.renderBlit)
			{
				...
				_openflRect2.setTo(0, 0, graphic.width, graphic.height);
				camera.copyPixels(frame, framePixels, _openflRect2, _ppoint);
			}
			else
			{
				if (_tileFrame == null)
	...
	function regenGraphic():Void
	{
		...
		if (FlxG.renderBlit)
		{
			pixels.lock();
			_openflRect2.setTo(0, 0, graphic.width, graphic.height);
			pixels.fillRect(_openflRect2, FlxColor.TRANSPARENT);
			_matrix.identity();
			_matrix.scale(sx, sy);
