...
class FlxShapeCross extends FlxShape
{
	...
	public override function drawSpecificShape(?matrix:Matrix):Void
	{
		...
		_openflRect.x = (horizontalLength - verticalThickness) * intersectionH; // line up the vertical rectangle with center
		_openflRect.y = 0;
		_openflRect.width = verticalThickness;
		_openflRect.height = verticalLength;

		// fr2 == horizontal rectangle
		_openflRect2.x = 0;
		_openflRect2.y = (verticalLength - horizontalThickness) * intersectionV;
		_openflRect2.width = horizontalLength;
		_openflRect2.height = horizontalThickness;

		// Copy vertices from the two rectangles
		vertices[0].y = _openflRect.top;
		vertices[0].x = _openflRect.left; // top-left of vertical beam
		vertices[1].y = _openflRect.top;
		vertices[1].x = _openflRect.right; // top-right
		vertices[6].y = _openflRect.bottom;
		vertices[6].x = _openflRect.right; // bottom-right
		vertices[7].y = _openflRect.bottom;
		vertices[7].x = _openflRect.left; // bottom-left

		// Copy vertices from the two rectangles
		vertices[3].y = _openflRect2.top;
		vertices[3].x = _openflRect2.right; // top-right of horizontal beam
		vertices[4].y = _openflRect2.bottom;
		vertices[4].x = _openflRect2.right; // bottom-right
		vertices[9].y = _openflRect2.bottom;
		vertices[9].x = _openflRect2.left; // bottom-left
		vertices[10].y = _openflRect2.top;
		vertices[10].x = _openflRect2.left; // top-left
