...
class FlxShaderMaskCamera extends FlxCamera
{
	...
	public function new(effectShader:Shader, X:Int = 0, Y:Int = 0, Width:Int = 0, Height:Int = 0, Zoom:Float = 0)
	{
		...
		openflSprite.addChild(_shadedScrollRect);
	...
	{
		FlxDestroyUtil.removeChild(openflSprite, _shadedScrollRect);
	...
	override function drawFX():Void
	{
		...
		{
			var alphaComponent = _fxopenflColor.alpha;
			super.fill((_fxopenflColor & 0x00ffffff), true, ((alphaComponent <= 0) ? 0xff : alphaComponent) * _fxopenflAlpha / 255, shaderCanvas.graphics);
		}
