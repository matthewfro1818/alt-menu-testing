 ...
 * Based on Kyle Pulver's NineSlice class for openflPunk: http://kpulv.com/96/openflpunk_NineSlice_Class__Updated__/
 * @author kpulv
 * @author Zaphod
 * @since  2.1.0
 */
class FlxSliceSprite extends FlxStrip
{
	...
	function regenGraphic():Void
	{
		...
		if (FlxG.renderBlit)
		{
			...
			{
				_openflRect2.setTo(0, 0, _snappedWidth, _snappedHeight);
				renderSprite.pixels.fillRect(_openflRect2, FlxColor.TRANSPARENT);
			}

			FlxSpriteUtil.openflGfx.clear();
			FlxSpriteUtil.openflGfx.beginBitmapFill(graphic.bitmap);
			FlxSpriteUtil.openflGfx.drawTriangles(vertices, indices, uvtData);
			FlxSpriteUtil.openflGfx.endFill();
			renderSprite.pixels.draw(FlxSpriteUtil.openflGfxSprite, null, colorTransform);
			FlxSpriteUtil.openflGfx.clear();
