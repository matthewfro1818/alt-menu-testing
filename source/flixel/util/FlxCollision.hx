...
class FlxCollision
{
	...
	static var openflRect:Rectangle = new Rectangle();
	...
	public static function pixelPerfectCheck(Contact:FlxSprite, Target:FlxSprite, AlphaTolerance:Int = 1, ?Camera:FlxCamera):Bool
	{
		...
		boundsA.copyToopenfl(openflRect);
		var pixelsA = testA.getPixels(openflRect);

		boundsB.copyToopenfl(openflRect);
		var pixelsB = testB.getPixels(openflRect);
