...
{
	/**
	 * Useful helper objects for doing openfl-specific rendering.
	 * Primarily used for "debug visuals" like drawing bounding boxes directly to the screen buffer.
	 */
	public static var openflGfxSprite(default, null):Sprite = new Sprite();

	public static var openflGfx(default, null):Graphics = openflGfxSprite.graphics;
	...
	public static function drawLine(sprite:FlxSprite, StartX:Float, StartY:Float, EndX:Float, EndY:Float, ?lineStyle:LineStyle, ?drawStyle:DrawStyle):FlxSprite
	{
		...
		openflGfx.moveTo(StartX, StartY);
		openflGfx.lineTo(EndX, EndY);
			...
			FillColor:FlxColor = FlxColor.TRANSPARENT, ?lineStyle:LineStyle, ?drawStyle:DrawStyle):FlxSprite
	{
		...
		openflGfx.moveTo(StartX, StartY);
		openflGfx.curveTo(EndX, EndY, ControlX, ControlY);
		endDraw(sprite, drawStyle);
		return sprite;
	}
	...
	{
		beginDraw(FillColor, lineStyle);
		openflGfx.drawRect(X, Y, Width, Height);
		endDraw(sprite, drawStyle);
		return sprite;
	}
	...
	{
		beginDraw(FillColor, lineStyle);
		openflGfx.drawRoundRect(X, Y, Width, Height, EllipseWidth, EllipseHeight);
		endDraw(sprite, drawStyle);
		return sprite;
	}

	#if (openfl || openfl >= "8.0.0")
	/**
	...
	{
		beginDraw(FillColor, lineStyle);
		openflGfx.drawRoundRectComplex(X, Y, Width, Height, TopLeftRadius, TopRightRadius, BottomLeftRadius, BottomRightRadius);
		endDraw(sprite, drawStyle);
		return sprite;
	}
	#end
			...
			?lineStyle:LineStyle, ?drawStyle:DrawStyle):FlxSprite
	{
		...
		beginDraw(FillColor, lineStyle);
		openflGfx.drawCircle(X, Y, Radius);
		endDraw(sprite, drawStyle);
		return sprite;
	}
	...
	{
		beginDraw(FillColor, lineStyle);
		openflGfx.drawEllipse(X, Y, Width, Height);
		endDraw(sprite, drawStyle);
		return sprite;
	}
	...
	{
		beginDraw(FillColor, lineStyle);
		openflGfx.moveTo(X + Height / 2, Y);
		openflGfx.lineTo(X + Height, Height + Y);
		openflGfx.lineTo(X, Height + Y);
		openflGfx.lineTo(X + Height / 2, Y);
		endDraw(sprite, drawStyle);
		return sprite;
	}
			...
			?drawStyle:DrawStyle):FlxSprite
	{
		...
		openflGfx.moveTo(p.x, p.y);
		for (p in Vertices)
		{
			openflGfx.lineTo(p.x, p.y);
		}
		endDraw(sprite, drawStyle);
		Vertices.unshift(p);
		return sprite;
	...
	public static inline function beginDraw(FillColor:FlxColor, ?lineStyle:LineStyle):Void
	{
		...
		{
			openflGfx.beginFill(FillColor.to24Bit(), FillColor.alphaFloat);
		}
	}
	...
	public static function updateSpriteGraphic(sprite:FlxSprite, ?drawStyle:DrawStyle):FlxSprite
	{
		...
		sprite.pixels.draw(openflGfxSprite, drawStyle.matrix, drawStyle.colorTransform, drawStyle.blendMode, drawStyle.clipRect, drawStyle.smoothing);
		sprite.dirty = true;
		return sprite;
	}
	...
	public static inline function setLineStyle(lineStyle:LineStyle):Void
	{
		if (lineStyle != null)
		{
			...
			openflGfx.lineStyle(lineStyle.thickness, color.to24Bit(), color.alphaFloat, lineStyle.pixelHinting, lineStyle.scaleMode, lineStyle.capsStyle,
