...
class FlxRect implements IFlxPooled
{
	...
	/**
	 * Helper function, just copies the values from the specified openfl rectangle.
	 *
	 * @param	openflRect	Any Rectangle.
	 * @return	A reference to itself.
	 */
	public inline function copyFromopenfl(openflRect:Rectangle):FlxRect
	{
		x = openflRect.x;
		y = openflRect.y;
		width = openflRect.width;
		height = openflRect.height;
		return this;
	}

	/**
	 * Helper function, just copies the values from this rectangle to the specified openfl rectangle.
	 *
	 * @param	Point	Any Rectangle.
	 * @return	A reference to the altered rectangle parameter.
	 */
	public inline function copyToopenfl(?openflRect:Rectangle):Rectangle
	{
		if (openflRect == null)
		{
			openflRect = new Rectangle();
		}

		openflRect.x = x;
		openflRect.y = y;
		openflRect.width = width;
		openflRect.height = height;
		return openflRect;
	}
